...
class evaluator:
    def __init__(self, args, model, test_loader, device, iterations=400):
        ...
        self.threshold = 5 #Default ripped from ContourPose
        ...
        # ablation
        # self.error_num = 0
        with open(os.path.join(os.getcwd(), 'diameter_dictionary.json')) as json_file:
            diameterDictionary = json.load(json_file)
        self.diameter = diameterDictionary[args.obj]

        # for PECP
        example = ""
        for i in range(0, self.keypoints.shape[0]):
            if i < 10:
    ...
    def calculate_metric(self, keypoints2d, predict2d, K):
        ...
        for i in range(batch_size):
            ...
            gt_pose = self.pnp(self.keypoints, keypoints, k)
    ...
    def calculate_metric_PECP(self, keypoints2d, predict2d, K, pose, pred_contour):
        ...
        for i in range(batch_size):
            ...
            if foreground > 1000:
                ...
                pred_pose = self.PECP(self.keypoints, predict, k, edge_heatmap, self.list_all)
            else:
                pred_pose = self.pnp(self.keypoints, predict, k)
            self.projection_2d(pred_pose, gt_pose, k)
            self.add_metric(pred_pose, gt_pose)
    ...
    def PECP(self, points_3d, points_2d, K, target_contour, list_all):
        ...
        for i in range(iteration_time):
            ...
            # 2d points
            valid_2d = self.project(self.valid_3d, K, pose).astype(int)
        ...
        for k in range(4, total):
            ...
            valid_2d = self.project(self.valid_3d, K, pose).astype(int)  # 得到2d点
            sum, valid_2d = self.get_confidence(target_contour, valid_2d)
            if sum >= max:
                max = sum
                pose_final = pose
        ...
        valid_2d = self.project(self.valid_3d, K, ransac_pose).astype(int)  # 得到2d点
        sum, valid_2d = self.get_confidence(target_contour, valid_2d)
        if sum >= max:
    ...
    def projection_2d(self, pose_pred, pose_targets, K):
        ...
        model_2d_pred = self.project(self.points, K, pose_pred)
        model_2d_targets = self.project(self.points, K, pose_targets)
    ...
    def add_metric(self, pose_pred, pose_targets, syn=False, percentage=0.1):
        ...
        model_pred = (
                np.dot(self.points, pose_pred[:, :3].T) + pose_pred[:, 3]
        )
        model_targets = (
                np.dot(self.points, pose_targets[:, :3].T) + pose_targets[:, 3]
        )
